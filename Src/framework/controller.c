...
#define MAX_CMD_BUF		60							// max number of user commands in the cmd buffer
#define MAX_KEYS		16							// number of physical buttons on this device

volatile usercmd_t 		uCmdBuffer[MAX_CMD_BUF];
volatile int 			uCmdIndex = 0;
volatile int  			uCmdOutdex = 0;
 ...
 *
 */
inline int QueueCmd( usercmd_t uCmd ) {
